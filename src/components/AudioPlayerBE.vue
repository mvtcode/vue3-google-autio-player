...
// URL mặc định sử dụng backend Express
const audioUrl = ref('http://localhost:3000/audio/1Giwm6I5XsW_ZOhoT9zrJgx68Yzro8yAD')
...
const handleUrlChange = async () => {
  ...
  try {
    if (audioPlayer.value) {
      // Nếu URL đã là backend URL, sử dụng trực tiếp
      if (audioUrl.value.includes('/audio/')) {
        audioPlayer.value.src = audioUrl.value
      } else {
        // Nếu là URL Google Drive, chuyển đổi thành backend URL
        const fileId = audioUrl.value.match(/[-\w]{25,}/)
        if (!fileId) {
          ElMessage.error('URL Google Drive không hợp lệ')
          return
        }
        const backendUrl = `http://localhost:3000/audio/${fileId[0]}`
        audioPlayer.value.src = backendUrl
      }
...
const downloadAudio = () => {
  ...
  window.open(`http://localhost:3000/audio/${fileId[0]}`, '_blank')
}
...
onMounted(() => {
  ...
  if (audioPlayer.value) {
    ...
    audioPlayer.value.addEventListener('error', (e) => {
      ...
      ElMessage.error('Có lỗi khi tải file audio. Vui lòng kiểm tra Express server đã chạy chưa.')
    })
    ...
    <el-card class="player-card">
      <template #header>
        <div class="card-header">
          <h3>Google Drive Audio Player (Backend Version)</h3>
          <small class="note"
            >*Lưu ý: File audio phải được chia sẻ công khai trên Google Drive và Express server phải
            đang chạy tại port 3000</small
          >
        </div>
      </template>
